...
//Copy all tagged fields to new object

//make command to apply body parts
//player doesn't disappear

function copyClient(%this)
{
	%newClient = new ScriptObject()
	{
		class = fClient;
	};
	...
	{
		%field1 = getField(%field,0);
		%field2 = getField(%field,1);
		eval(%newClient @ "." @ %field1 @ " = \"" @ %field2 @ "\";");
		%index++;
	}
...
function copyPlayer(%this)
{
	...
	while((%field = %this.getTaggedField(%index)) !$= "")
	{
		%field1 = getField(%field,0);
		%field2 = getField(%field,1);
		eval(%newPlayer @ "." @ %field1 @ " = \"" @ %field2 @ "\";");
		%index++;
	}
	%newPlayer.setTransform(%this.getTransform());
	%newPlayer.setDatablock(%this.getDatablock());
	
	return %newPlayer;
}

function fClient::applyBodyParts(%client)
{
...
function fClient::applyBodyColors(%client)
{
	if(!isObject(%client.player))
		return;
